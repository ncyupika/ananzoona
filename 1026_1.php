...
{
	//未審核
	$sql = "SELECT * FROM safereport WHERE 學年度 = '$year' && 導師複查 = 0 && 導師 = '$useraccount' order by 學年度 DESC";
	$rows = mysqli_query($db, $sql);
	$num = mysqli_num_rows ($rows);
	
	//已審核
	$sql_checked = "SELECT * FROM safereport WHERE 學年度 = '$year' && 導師複查 = 1 && 導師 = '$useraccount' order by 學年度 DESC";
...
else if($usertype == '超級使用者' || '教官')
{
	//未審核
	$sql = "SELECT * FROM safereport WHERE 學年度 = '$year' && 導師複查 = 1 && 教官複查 = 0 && 系所 = '$type' order by safe_level ASC";
	$rows = mysqli_query($db, $sql);
	$num = mysqli_num_rows ($rows);
	
	//已審核
	$sql_checked = "SELECT * FROM safereport WHERE 學年度 = '$year' && 教官複查	= 1 && 系所 = '$type' order by safe_level ASC";
	$rows_checked = mysqli_query($db, $sql_checked);
	$num_checked = mysqli_num_rows ($rows_checked);
	...
	<body class="is-preload">
		...
		<!-- Main -->
			<div id="main">
				...
				<!-- Header -->
					<header id="header">
						<h2><strong>安全回報-<?php echo $type;?></strong></h2>
							...
							<?php
								if($num >0)
								{
									for($i=0 ; $i<$num ; $i++)
									{
										...
										echo "<td>"."<a href ='safereport.php?name=$row[1]&year=$row[13]&group=$row[2]&type=$type'>$row3[0]</a>"."</td>";
								...
								if($num_checked >0)
								{
									for($i=0 ; $i<$per ; $i++)
									{
										...
										if($row_checked[1] != NULL)
										{
											...
											echo "<td>"."<a href ='safereport.php?name=$row_checked[1]&year=$row_checked[13]&group=$row_checked[2]&type=$type'>$row3[0]</a>"."</td>";
											...
											echo "<td>"."<a target='_blank' href='https://appr.tc/r/ncyumis_s".$row_group[0]."'>點我聯絡室長</a></td>";
